...
def generate_figure6(conn, pdf):
    ...
    try:
        ...
        if irrigation_df.empty:
            print("No irrigation events found in database")
            irrigation_df = pd.DataFrame(columns=['date', 'amount_mm'])
        else:
        ...
        # Verify date ranges
        date_min = cwsi_df['date'].min()
        date_max = cwsi_df['date'].max()
        print(f"Data spans from {date_min.date()} to {date_max.date()}")
        # Assuming all dates are within 2024 as per the terminal output
        ...
        # Add regression line using numpy for efficiency
        slope, intercept, r_value, p_value, std_err = stats.linregress(indices_df['swsi'], indices_df['cwsi'])
        ax1.plot(indices_df['swsi'], intercept + slope * indices_df['swsi'], color=CUSTOM_COLORS['red'])
        
        # Add correlation coefficient
        corr = r_value  # Already calculated
        ...
        # Vectorized plotting of irrigation events
        if not irrigation_df.empty:
            print("Adding irrigation markers...")
            # Plot vertical lines for irrigation events
            ax2.vlines(
                irrigation_df['date'],
                ymin=ax2.get_ylim()[0],
                ymax=ax2.get_ylim()[1],
            ...
            # Optionally, add a single legend entry for irrigation
            # Remove duplicate labels if any
            handles, labels = ax2.get_legend_handles_labels()
            by_label = dict(zip(labels, handles))
            ax2.legend(by_label.values(), by_label.keys(), loc='upper right')
        else:
            print("No irrigation events to plot.")
        ...
        irrigation_dates = irrigation_df['date'].dt.date.unique()
        irrigation_indices = indices_df[indices_df['date'].dt.date.isin(irrigation_dates)].copy()
        print(f"Found {len(irrigation_indices)} days with irrigation events")
        
        if not irrigation_indices.empty:
            # Scatter plots for irrigation days
            ax3.scatter(
            ...
            print("No irrigation indices to plot.")
